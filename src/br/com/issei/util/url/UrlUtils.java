...
public final class UrlUtils {
	 ...
	 *             {@value #ENCODING_CHARSET}.
	 */
	public static Map<String, List<String>> extractParametersFromQueryString(
			String queryString) {
		if (queryString == null)
			return emptyMap();

		// If there is no ? character at the front of the string, append it.
		return extractParametersFromUrl(format("restfb://url%s",
	 ...
	 * @return A mapping of query string parameter names to values. If
	 *         {@code url} is {@code null}, an empty {@code Map} is returned.
	 * @throws IllegalStateException
	 *             If unable to URL-decode because the JVM doesn't support
	 *             {@value #ENCODING_CHARSET}.
	 */

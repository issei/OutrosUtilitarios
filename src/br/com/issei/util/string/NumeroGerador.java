 ...
 */
public class NumeroGerador {

	private NumeroGerador() {
	}
	 ...
	 * @return
	 */
	public static int[] generateArray(Random random, int length, int quantityOfZeros) {
	...
	public static int generateRandom(int maxLength) {
		long l = CPFGeradorUtil.arrayToLong(generateRandomArray(maxLength, maxLength));
	...
	public static int generateRandomForce(int length) {
		long l = CPFGeradorUtil.arrayToLong(generateRandomArray(length, 0));
		if (l > Integer.MAX_VALUE) {
			return Integer.MAX_VALUE;			
		}
		return (int) l;
